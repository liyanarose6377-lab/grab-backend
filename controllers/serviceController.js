...
exports.grabService = async (req,res) => {
  ...
  if(user.balance < amount) return res.status(400).json({msg:'insufficient'});
  user.balance -= amount; // reserve
  await user.save();
  const order = await ServiceOrder.create({ user:userId, serviceName, amount, status:'queued' });
